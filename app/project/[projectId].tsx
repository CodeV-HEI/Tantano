...
const TransactionCard = ({
    ...
    <View
        className={`flex-row justify-between items-center p-3 mb-2 rounded-lg ${theme === "dark" ? "bg-gray-800" : "bg-gray-100"
            }`}
    >
        <View className="flex-1">
            ...
            <Text
                className={`text-sm ${theme === "dark" ? "text-cyan-400" : "text-cyan-600"
                    }`}
            >
                Coût estimé: {transaction.estimatedCost.toLocaleString()} Ar
            </Text>
            {transaction.realCost !== undefined && (
                ...
                >
                    Coût réel: {transaction.realCost.toLocaleString()} Ar
                </Text>
            )}
        </View>
...
export default function ProjectDetailScreen() {
    ...
    const [currentTransaction, setCurrentTransaction] = useState<
        CreationProjectTransaction & { id?: string }
    >({ name: "", estimatedCost: 0 });
    ...
    const fetchProjectDetails = useCallback(async () => {
        ...
        try {
            ...
            const transactionsResponse = await projectAPI.getAllTransactions(
                accountId,
                projectId as string
            );
            ...
            Toast.show({
                type: "error",
                text1: "Erreur",
                text2: "Impossible de charger le projet.",
            });
            router.back();
        } finally {
            setLoading(false);
    ...
    const handleArchiveProject = async () => {
        ...
        try {
            ...
            Toast.show({
                type: "error",
                text1: "Erreur",
                text2: "Impossible de modifier le statut.",
            });
        } finally {
            setLoading(false);
        }
        ...
        (async () => {
            ...
            try {
                ...
                Toast.show({
                    type: "error",
                    text1: "Erreur",
                    text2: "Impossible de supprimer le projet.",
                });
            } finally {
                setLoading(false);
            }
    ...
    const handleUpdateProject = async () => {
        ...
        try {
            ...
            Toast.show({
                type: "error",
                text1: "Erreur",
                text2: "Impossible de mettre à jour le projet.",
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleAddTransaction = async () => {
        ...
        setLoading(true);
        try {
            await projectAPI.createTransaction(
                accountId,
                projectId as string,
                currentTransaction
            );
    ...
    const handleEditTransaction = async () => {
        ...
        setLoading(true);
        try {
            await projectAPI.updateTransaction(
                accountId,
                projectId as string,
    ...
    const handleDeleteTransaction = (transactionId: string) => {
        ...
        (async () => {
            try {
                await projectAPI.deleteTransaction(
                    accountId,
                    projectId as string,
                    transactionId
                );
    ...
    return (
        <View className={`flex-1 ${theme === "dark" ? "bg-black" : "bg-white"}`}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                {/* Project Header */}
                <View
                    className={`p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"
                ...
                >
                    <View className="flex-row items-center justify-between mb-4">
                        ...
                        <TouchableOpacity
                            onPress={() => router.back()}
                            className="mr-3 p-2 rounded-full bg-gray-200 dark:bg-gray-700"
                        >
                            <MaterialIcons
                                name="arrow-back"
                                size={24}
                    ...
                    >
                        <View className="flex-row justify-between mb-3">
                            ...
                            <Text
                                className={`font-bold ${theme === "dark" ? "text-cyan-400" : "text-cyan-600"
                                    }`}
                            >
                        ...
                        {statistics && (
                            <>
                                <View className="flex-row justify-between mb-3">
                                    ...
                                    <Text
                                        className={`font-bold ${theme === "dark" ? "text-purple-400" : "text-purple-600"
                                            }`}
                                    >
                                        {statistics.totalEstimatedCost.toLocaleString()} Ar
                                    </Text>
                                </View>
                                <View className="flex-row justify-between">
                                    ...
                                    <Text
                                        className={`font-bold ${statistics.remainingBudget >= 0
                                            ? "text-green-500"
                                            : "text-red-500"
                                            }`}
                                    >
                                        {statistics.remainingBudget.toLocaleString()} Ar
                                    </Text>
                                </View>
            ...
            {/* Modals (inchangées mais adaptées au thème) */}
            {/* Edit Project Modal */}
            <Modal
                    ...
                    >
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <TextInput
                                ...
                                onChangeText={(text) =>
                                    editedProject && setEditedProject({ ...editedProject, name: text })
                                }
                            ...
                            <TextInput
                                ...
                                placeholderTextColor="#9ca3af"
                                value={
                                    editedProject?.initialBudget === 0
                                        ? ""
                                        : editedProject?.initialBudget.toString()
                                }
                                onChangeText={(text) =>
                                    editedProject &&
                                    setEditedProject({
                    ...
                    >
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <TextInput
                                ...
                                onChangeText={(text) =>
                                    setCurrentTransaction({ ...currentTransaction, name: text })
                                }
                                className={`px-4 py-3 rounded-xl mb-4 text-lg ${theme === "dark"
                                    ? "bg-gray-800 text-white"
                                    : "bg-gray-100 text-gray-900"
                            ...
                            <TextInput
                                ...
                                placeholderTextColor="#9ca3af"
                                value={
                                    currentTransaction.estimatedCost === 0
                                        ? ""
                                        : currentTransaction.estimatedCost.toString()
                                }
                                onChangeText={(text) =>
                                    setCurrentTransaction({
                                        ...currentTransaction,
