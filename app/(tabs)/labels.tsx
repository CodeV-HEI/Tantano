...
export default function LabelsScreen() {
    ...
    }, [fetchLabels, searchLabels, searchQuery, user?.id]);
    ...
    const handleUpdateLabel = async (name: string, color: string) => {
        ...
        console.log(' Mise à jour avec couleur:', { name, color });
        const updated = await updateLabel(user.id, editingId, name, color);
        if (updated) {
            setEditingId(null);
            setEditingName('');
            setEditingColor('');
    ...
    const handleEditClick = (id: string, currentName: string, currentColor: string) => {
        setEditingId(id);
                ...
                <View className="px-4 pt-8 pb-8">
                    ...
                    {editingId && (
                        <EditForm
                            ...
                            isUpdating={isUpdating[editingId] || false}
